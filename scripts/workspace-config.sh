...
mkdir build
cd build
bash -c "cmake -DCMAKE_POSITION_INDEPENDENT_CODE=TRUE -DCMAKE_POLICY_VERSION_MINIMUM=3.5 .."    # set -fPIC option; otherwise qpOASES can't be linked by shared lib
make -j$(nproc) && sudo make install
